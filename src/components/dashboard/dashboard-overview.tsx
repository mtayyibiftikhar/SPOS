...
export function DashboardOverview() {
  ...
  const connectedDevices = currentShop
    ...
    : 0;
  const dashboardAnnouncement = state.brand.loginAdEnabled ? (
      ...
      <div className="grid gap-0 lg:grid-cols-[0.72fr_1.28fr]">
        <div className="relative min-h-48 overflow-hidden bg-slate-950">
          {state.brand.loginAdImageUrl ? (
            ...
            <div className="grid h-full min-h-48 place-items-center bg-[radial-gradient(circle_at_top_left,_rgba(16,185,129,0.42),_transparent_34%),linear-gradient(135deg,#020617_0%,#064e3b_100%)]">
              <Sparkles className="h-10 w-10 text-emerald-100" />
            </div>
          )}
          <div className="absolute left-4 top-4 inline-flex items-center gap-2 rounded-full bg-white/90 px-3 py-2 text-[11px] font-bold uppercase tracking-[0.16em] text-emerald-800 shadow-soft">
        ...
        <div className="flex flex-col justify-center p-5 lg:p-7">
          ...
          <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600">{state.brand.loginAdMessage}</p>
          {state.brand.loginAdCtaLabel && state.brand.loginAdCtaUrl ? (
            <a
              className="mt-5 inline-flex w-fit items-center gap-2 rounded-2xl bg-slate-950 px-5 py-3 text-sm font-semibold text-white transition hover:bg-emerald-700"
